...
try:
    ...
    print("ОШИБКА: Не установлена библиотека PyQt5.")
    sys.exit(1)
...
if not BINARY_PATH:
    for p in ["/usr/bin/adguardvpn-cli", "/usr/local/bin/adguardvpn-cli", "/opt/adguardvpn/bin/adguardvpn-cli"]:
        if os.path.exists(p):
            BINARY_PATH = p
            break
...
class Worker(QThread):
    ...
    def run(self):
        try:
            ...
            if self.needs_root:
                env = f"DISPLAY={os.environ.get('DISPLAY', ':0')} XAUTHORITY={os.environ.get('XAUTHORITY', '')} HOME={USER_HOME}"
                final_cmd = f"pkexec env {env} {cmd_exec}"
            else:
                final_cmd = cmd_exec
...
class AdGuardVPNGUI(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def init_ui(self):
        ...
        self.btn_disconnect.setStyleSheet("background-color: #C62828; color: white; font-weight: bold; border-radius: 4px;")
        ...
        self.btn_login.setStyleSheet("background-color: #FF9800; color: white; font-weight: bold;")
        ...
        self.btn_install.setStyleSheet("background-color: #0288D1; color: white; font-weight: bold;")
    ...
    def set_buttons_logic(self):
        if "CONNECTED" in self.status_label.text() and "DIS" not in self.status_label.text():
    ...
    def show_install_instruction(self):
        cmd = "curl -s https://raw.githubusercontent.com/AdguardTeam/AdGuardVPNCLI/master/scripts/install.sh | sh"
        InstructionDialog("Установка", "Выполните эту команду в терминале:", cmd).exec_()

    def show_login_instruction(self):
        cmd = f"{BINARY_NAME} login"
        InstructionDialog("Вход", "Откройте терминал и выполните команду входа:", cmd).exec_()
    ...
    def on_login_checked(self, success, output):
        if not success and ("login" in output.lower() or "auth" in output.lower()):
            ...
            if success: self.parse_locations(output)
            self.check_status_routine()

    def check_status_routine(self):
        if not self.is_logged_in: return
        if "Ввод пароля" in self.status_label.text(): return 
        if hasattr(self, 'w_status') and self.w_status.isRunning(): return
    ...
    def parse_and_apply_status(self, output):
        ...
        if "disconnected" in text or "vpn stopped" in text:
        ...
        elif "connected" in text and ("successfully" in text or "mode" in text or "to" in text):
             if "CONNECTED" not in self.status_label.text():
                 ...
                 if "connected to" in text:
                     ...
                     if len(parts) > 1:
                         rest = parts[1]
                         if " in " in rest: city = rest.split(" in ")[0].strip()
                         else: city = rest.split("\n")[0].strip()
    ...
    def parse_locations(self, output):
        ...
        for line in output.split('\n'):
            ...
            id_code = parts[0]
            if len(id_code) != 2 or not id_code.isalpha(): continue
            ping_str = parts[-1]
            if not ping_str.isdigit(): continue
            locations.append((int(ping_str), f"[{ping_str} ms]  {' '.join(parts[1:-1])} ({id_code})", id_code))

        locations.sort(key=lambda x: x[0])
        for _, display, code in locations:
            self.combo.addItem(display, code)
        if self.current_city: self.select_combo_text(self.current_city)
    ...
    def on_act_done(self, success, output):
        if success:
            self.parse_and_apply_status(output)
            if "CONNECTED" not in self.status_label.text() and "DISCONNECTED" not in self.status_label.text():
    ...
    def closeEvent(self, event):
        is_connected_strict = ("CONNECTED" in self.status_label.text() and "DIS" not in self.status_label.text())
        ...
        if is_connected_strict:
            ...
            if reply == QMessageBox.Yes:
                ...
                if BINARY_PATH: cmd_exec = cmd_exec.replace(BINARY_NAME, BINARY_PATH, 1)
                
                # Используем nohup, чтобы процесс не умер вместе с Python
                # start_new_session=True отвязывает процесс от терминала
                env = f"DISPLAY={os.environ.get('DISPLAY', ':0')} XAUTHORITY={os.environ.get('XAUTHORITY', '')} HOME={USER_HOME}"
                final_cmd = f"nohup pkexec env {env} {cmd_exec} >/dev/null 2>&1 &"
                
                subprocess.Popen(final_cmd, shell=True, start_new_session=True)
